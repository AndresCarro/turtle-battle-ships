...
export const setupGameSockets = (io: Server) => {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("join-game", async (data: { gameId: number; username: string }) => {
      try {
        ...
        if (game.player1 !== username && game.player2 !== username) {
          socket.emit("error", { message: "You cannot join this game, the room is full" });
          return;
        }
        ...
        console.log(`Player ${username} joined game ${gameId} (socket: ${socket.id})`);
...
export const emitToGameRoom = (io: Server, gameId: number, event: string, data: any) => {
  const roomName = `game-${gameId}`;
  io.to(roomName).emit(event, data);
};

// Helper function to emit to a specific player in a game
export const emitToPlayer = (io: Server, gameId: number, username: string, event: string, data: any) => {
  const roomName = `game-${gameId}`;
